...
(function () {
  ...
  function showBakedTrailheadStill(done) {
    ...
    var VIDEO_FADE_MS = 220;
    var BAKE_FADE_MS = 480;
    ...
    function crossfadeBakeOntoFreeze(img, then) {
      ...
      function tick(now) {
        ...
        /* ease-out cubic — smooth settle, no end pop */
    ...
    function beginSoftEnd(img) {
      /* Reveal freeze under video with a short opacity fade (frames match → zero jump). */
      if (video) {
        try {
          video.style.transition = "opacity " + VIDEO_FADE_MS + "ms ease";
          video.style.opacity = "0";
        } catch (e) {}
      }
      setTimeout(function () { afterVideoSoftHide(img); }, VIDEO_FADE_MS + 20);
    }
