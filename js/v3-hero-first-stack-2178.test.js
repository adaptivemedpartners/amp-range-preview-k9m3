...
function checkPage(rel) {
  ...
  assert(html.indexOf("amp-build 2186-specialty-ctx-photo-taller") !== -1, rel + " chip");
  ...
  assert(hero < stripAt && stripAt < rotator && rotator < bridge && bridge < finding && finding < steps && steps < row, rel + " specialty strip under the hero, stay line back in the bridge, then row2");
  assert(hero < eyebrow && eyebrow < place && place < sub && sub < stripAt && stripAt < rotator, rel + " specialty strip sits under the hero sub");
  ...
  assert(strip.indexOf('aria-label="Who we place: Physicians, APPs, Dental, Behavioral Health, Allied Health"') !== -1, rel + " who we place label");
